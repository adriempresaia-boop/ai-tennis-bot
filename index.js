// index.js — TwentyBet Full Ace Bot
...
function norm(s){ if(!s) return ''; s=s.trim(); if(ALIASES[s]) return ALIASES[s];
...
function throttle(){ if(!STATE.lastAlertAt) return false;
  const diff=(Date.now()-new Date(STATE.lastAlertAt).getTime())/60000; return diff<parseInt(process.env.ALERT_MIN_COOLDOWN_MIN||'10',10); }
async function alert({mid,owner,len,min}){
  const txt=`🎾 Racha alcanzada\n• Enfrentamiento: ${mid}\n• Ganador actual: ${owner}\n• Racha: ${len}\n• Umbral: ${min}\n• ${new Date().toISOString()}`;
  if(!process.env.ALERT_WEBHOOK_URL){ console.log('[ALERTA]',txt); return; }
  const headers={'Content-Type':'application/json'};
  if(process.env.ALERT_WEBHOOK_BEARER) headers.Authorization=`Bearer ${process.env.ALERT_WEBHOOK_BEARER}`;
  await fetch(process.env.ALERT_WEBHOOK_URL,{method:'POST',headers,body:JSON.stringify({text:txt})});
...
function computeTables(rows){
  ...
  const LAST_N=parseInt(process.env.STREAK_LAST_N||'10',10);
...
async function scrapeOnce(page,s){
  ...
  for(const card of cards){
    ...
    for(const m of MATCHUPS){
      ...
      if(new RegExp(`\\b${A}\\b`,'i').test(text) && new RegExp(`\\b${B}\\b`,'i').test(text)){ pair={a:A,b:B}; break; }
    }
    if(!pair) continue;
  ...
  const all=await read(s,'Results!A:I'); const rows=all.slice(1);
...
async function main(){
  ...
  while(true){
    try{ await scrapeOnce(page,s); } catch(e){ console.error('scrapeOnce error:', e && e.stack || e); }
